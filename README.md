# claw-doh-proxy
基于 Caddy 和 ClawCloud Run 的轻量化隐私保护 DoH 转发器，支持通过 NextDNS 实现自动国内外流量分流。 A lightweight, privacy-focused DNS-over-HTTPS (DoH) forwarder based on Caddy and ClawCloud Run, supporting automatic domestic/foreign traffic splitting via NextDNS."。
Claw-DoH-Proxy
